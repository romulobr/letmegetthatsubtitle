...
os.prototype.computeHash = function(path, cb) {
  ...
  this.on(path+'checksum-ready', function(chksum_part, name) {
  ...
  fs.stat(path, function(err, stat) {
    ...
    file_size = stat.size;
    self.emit(path+'checksum-ready', file_size.toString(16), "filesize");
    ...
    fs.open(path, 'r', function(err, fd) {
      ...
      for(var i in t_buffers) {
        fs.read(fd, t_buffers[i].buf, 0, chunk_size*2, t_buffers[i].offset, function(err, bytesRead, buffer) {
          ...
          //console.log(buffer);
          self.emit(path+'checksum-ready', self.checksumBuffer(buffer, 16), "buf?");
        });
      }
    });
  });
